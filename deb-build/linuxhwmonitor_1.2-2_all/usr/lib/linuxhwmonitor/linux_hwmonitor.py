#!/usr/bin/env python3
"""
LinuxHWMonitor - Monitor de Hardware para Linux
Inspirado en CrystalDiskInfo + HWiNFO64
Requiere: pip install PyQt5 psutil
Opcional: sudo apt install smartmontools lm-sensors
"""
    ...
    try:
        ...
        for dev in data.get("blockdevices", []):
            if dev.get("type") == "disk":
                disks.append({
                    "name":  dev.get("name", ""),
                    "model": (dev.get("model") or "Desconocido").strip(),
                    "size":  dev.get("size", "?"),
...
# ─────────────────────────────────────────────
#  DICCIONARIO S.M.A.R.T. EN ESPAÑOL
#  id -> (nombre corto en español, qué significa)
# ─────────────────────────────────────────────
SMART_ATTRS_ES = {
    1:   ("Tasa de errores de lectura",      "Frecuencia de errores al leer datos; valores raw altos pueden indicar deterioro."),
    2:   ("Rendimiento de transferencia",    "Rendimiento general del disco (informativo)."),
    3:   ("Tiempo de arranque",              "Tiempo que tardan los platos en alcanzar su velocidad (solo HDD)."),
    4:   ("Ciclos de arranque/paro",         "Veces que los platos han arrancado y parado (solo HDD)."),
    5:   ("Sectores reasignados",            "Sectores dañados reemplazados por sectores de reserva. Clave para la salud; >0 vigilar."),
    7:   ("Tasa de errores de búsqueda",     "Errores al posicionar el cabezal (solo HDD)."),
    8:   ("Rendimiento de búsqueda",         "Rendimiento de las operaciones de búsqueda (solo HDD)."),
    9:   ("Horas encendido",                 "Total de horas que el disco ha estado en funcionamiento."),
    10:  ("Reintentos de giro",              "Veces que hubo que reintentar arrancar los platos (solo HDD)."),
    11:  ("Reintentos de calibración",       "Reintentos de recalibración del disco (solo HDD)."),
    12:  ("Veces encendido",                 "Número de ciclos completos de encendido y apagado."),
    13:  ("Errores de lectura suave",        "Errores de lectura corregidos por software."),
    148: ("Atributo del fabricante",         "Atributo interno específico del fabricante."),
    149: ("Atributo del fabricante",         "Atributo interno específico del fabricante."),
    160: ("Errores no corregibles",          "Errores que el disco no pudo corregir."),
    167: ("Modo protección escritura",       "Estado del modo de protección contra escritura de la NAND."),
    168: ("Errores capa física SATA",        "Errores en la capa física del enlace SATA."),
    169: ("Tasa de bloques defectuosos",     "Proporción de bloques NAND defectuosos respecto a la reserva."),
    170: ("Bloques defectuosos (temp/tard)", "Conteo de bloques defectuosos detectados temprano y tarde."),
    171: ("Fallos de programación",          "Errores al escribir (programar) datos en la NAND."),
    172: ("Fallos de borrado",               "Errores al borrar bloques de la NAND."),
    173: ("Ciclos de borrado máx/prom",      "Ciclos de borrado promedio y máximo por bloque. Indica desgaste de la NAND."),
    174: ("Cortes inesperados",              "Veces que el disco perdió energía sin apagado seguro."),
    175: ("Fallos de programación (chip)",   "Errores de escritura a nivel de chip NAND."),
    176: ("Fallos de borrado (chip)",        "Errores de borrado a nivel de chip NAND."),
    177: ("Desgaste de nivelación",          "Indicador del desgaste de las celdas NAND por nivelación."),
    178: ("Bloques reserva usados (chip)",   "Bloques de reserva ya consumidos a nivel de chip."),
    179: ("Bloques reserva usados (total)",  "Bloques de reserva ya consumidos en todo el disco."),
    180: ("Bloques reserva libres",          "Bloques de reserva todavía disponibles."),
    181: ("Fallos de programación (total)",  "Total de errores al escribir datos en la NAND."),
    182: ("Fallos de borrado (total)",       "Total de errores al borrar bloques de la NAND."),
    183: ("Errores enlace SATA",             "Caídas/errores de negociación del enlace SATA."),
    184: ("Error extremo a extremo",         "Errores de integridad entre el caché interno y el host."),
    187: ("Errores no corregibles",          "Errores que no se pudieron corregir. Valores >0 son señal de alerta."),
    188: ("Tiempos de espera de comando",    "Comandos que excedieron el tiempo de espera."),
    189: ("Escrituras fuera de rango",       "Escrituras realizadas con el cabezal a altura incorrecta (solo HDD)."),
    190: ("Temperatura (flujo de aire)",     "Temperatura medida por el sensor de flujo de aire."),
    191: ("Errores por golpes",              "Errores provocados por golpes o vibración (solo HDD)."),
    192: ("Apagados inseguros",              "Apagados sin secuencia segura o retracciones de emergencia."),
    193: ("Ciclos de carga/descarga",        "Ciclos de carga y descarga del cabezal (solo HDD)."),
    194: ("Temperatura",                     "Temperatura actual del disco en grados Celsius."),
    195: ("Errores corregidos por ECC",      "Errores de hardware corregidos por el código ECC."),
    196: ("Eventos de reasignación",         "Veces que se intentó reasignar sectores dañados."),
    197: ("Sectores pendientes",             "Sectores inestables esperando reasignación. >0 indica datos en riesgo."),
    198: ("Sectores incorregibles",          "Sectores que no se pueden leer ni corregir."),
    199: ("Errores CRC de interfaz",         "Errores de transmisión por el cable SATA. Suele ser cable o conexión."),
    200: ("Errores multi-zona",              "Errores de escritura en varias zonas (solo HDD)."),
    201: ("Errores de lectura suave",        "Errores de lectura corregidos por software."),
    202: ("Vida útil restante",              "Porcentaje de vida útil restante del disco."),
    231: ("Vida útil del SSD restante",      "Porcentaje de vida útil restante de la memoria NAND."),
    232: ("Espacio de reserva disponible",   "Porcentaje de bloques de reserva todavía disponibles."),
    233: ("Resistencia / NAND escrita",      "Indicador de desgaste o cantidad de datos escritos a la NAND."),
    234: ("NAND escrita / borrada",          "Cantidad de datos escritos o borrados en la NAND."),
    235: ("Eventos de recuperación POR",     "Recuperaciones tras corte de energía."),
    241: ("Escrituras totales (Host)",       "Cantidad total de datos escritos al disco por el sistema."),
    242: ("Lecturas totales (Host)",         "Cantidad total de datos leídos del disco por el sistema."),
    244: ("Ciclos de borrado promedio",      "Promedio de ciclos de borrado por bloque NAND."),
    245: ("Ciclos de borrado máximo",        "Máximo de ciclos de borrado en un bloque NAND."),
    246: ("Ciclos de borrado totales",       "Total acumulado de ciclos de borrado de la NAND."),
    249: ("Escrituras a la NAND",            "Cantidad total de datos escritos físicamente a la NAND."),
}

# Pseudo-atributos NVMe (se identifican por nombre)
SMART_NVME_ES = {
    "Critical Warning":        ("Advertencia crítica",          "Banderas de advertencia crítica del disco. Debe ser 0."),
    "Temperature":             ("Temperatura",                  "Temperatura actual del disco."),
    "Available Spare":         ("Reserva disponible",           "Porcentaje de bloques de reserva disponibles."),
    "Available Spare Thresh":  ("Umbral de reserva",            "Porcentaje mínimo de reserva antes de alertar."),
    "Percentage Used":         ("Porcentaje de vida usada",     "Porcentaje de vida útil consumida según el fabricante."),
    "Data Units Read":         ("Lecturas totales (Host)",      "Unidades de datos leídas por el sistema."),
    "Data Units Written":      ("Escrituras totales (Host)",    "Unidades de datos escritas por el sistema."),
    "Host Read Commands":      ("Comandos de lectura",          "Número total de comandos de lectura recibidos."),
    "Host Write Commands":     ("Comandos de escritura",        "Número total de comandos de escritura recibidos."),
    "Controller Busy Time":    ("Tiempo de controlador ocupado","Minutos que el controlador estuvo ocupado."),
    "Power Cycles":            ("Veces encendido",              "Número de ciclos de encendido y apagado."),
    "Power On Hours":          ("Horas encendido",              "Total de horas que el disco ha estado funcionando."),
    "Unsafe Shutdowns":        ("Apagados inseguros",           "Apagados sin secuencia segura."),
    "Media Errors":            ("Errores de medio",             "Errores de integridad de datos detectados. Debe ser 0."),
    "Num Error Log Entries":   ("Entradas en registro errores", "Número de entradas en el registro interno de errores."),
}


def _attr_to_bytes(name, raw, lba_size=512):
    """Convierte el raw de un atributo de escritura/lectura a bytes según su nombre."""
    try:
        raw = int(raw)
    ...
    if raw <= 0:
        return None
    n = (name or "").lower()
    if "gib" in n:               return raw * (1024 ** 3)
    if "32mib" in n:             return raw * 32 * (1024 ** 2)
    if "mib" in n:               return raw * (1024 ** 2)
    if "gb" in n:                return raw * 1_000_000_000
    if "lba" in n or "sector" in n:
        return raw * lba_size
    # Por defecto (241/242 sin pista) se asume conteo de LBAs
    return raw * lba_size


def _fmt_bytes(b):
    """Formatea bytes a TB/GB legible (estilo CrystalDiskInfo)."""
    if b is None:
        return None
    tb = b / 1e12
    if tb >= 1:
...
def get_smart_data(dev_path):
    ...
    result = {
        ...
        "power_on_count": None,
        "total_writes": None,
        "total_host_writes": None,   # bytes
        "total_host_reads": None,    # bytes
        "total_nand_writes": None,   # bytes
        "rotation_rate": None,
        "standard_str": "",          # versión ATA / SATA real
        "transfer_mode": "",         # velocidad de interfaz real
    ...
    # Versión ATA / SATA reales (en vez de valores fijos)
    av = (data.get("ata_version") or {}).get("string", "")
    sv = (data.get("sata_version") or {}).get("string", "")
    if av and sv:
        result["standard_str"] = f"{av}  ·  {sv}"
    else:
        result["standard_str"] = av or sv or ""

    # Velocidad de interfaz real
    isp = data.get("interface_speed") or {}
    cur = (isp.get("current") or {}).get("string", "")
    mx  = (isp.get("max") or {}).get("string", "")
    if cur and mx and cur != mx:
        result["transfer_mode"] = f"{cur}  (máx {mx})"
    else:
        result["transfer_mode"] = cur or mx or ""

    # Interfaz más legible
    proto = data.get("device", {}).get("protocol", "")
    if proto == "ATA" and sv:
        result["interface"] = "Serial ATA (SATA)"
    elif proto:
        result["interface"] = proto

    # Características reales
    feats = ["S.M.A.R.T."]
    if (data.get("trim") or {}).get("supported"):
        feats.append("TRIM")
    rr = data.get("rotation_rate")
    if rr in (0, None) and proto:
        feats.append("SSD")
    elif isinstance(rr, int) and rr > 0:
        feats.append(f"{rr} RPM")
    if (data.get("ata_security") or {}).get("enabled") is not None:
        feats.append("Cifrado ATA")
    ...
    # SATA attributes
    lba_size = data.get("logical_block_size") or 512
    ...
    for a in attrs:
        ...
        if worst <= thresh and thresh > 0:
            flag = "Precaución" if val > thresh else "Malo"
    ...
    # SATA: escrituras / lecturas totales (Host) y NAND — estilo CrystalDiskInfo
    for a in attrs:
        aid  = a.get("id")
        nm   = (a.get("name") or "")
        low  = nm.lower()
        rawv = a.get("raw", {}).get("value", 0)
        if result["total_host_writes"] is None and (
            aid == 241 or "lifetime_writes" in low or "total_lbas_written" in low
            or "host_writes" in low or "total_writes" in low):
            result["total_host_writes"] = _attr_to_bytes(nm, rawv, lba_size)
        elif result["total_host_reads"] is None and (
            aid == 242 or "lifetime_reads" in low or "total_lbas_read" in low
            or "host_reads" in low or "total_reads" in low):
            result["total_host_reads"] = _attr_to_bytes(nm, rawv, lba_size)
        if result["total_nand_writes"] is None and (
            aid == 249 or ("nand" in low and ("writ" in low or "gb" in low or "gib" in low))
            or ("flash" in low and "writ" in low)):
            result["total_nand_writes"] = _attr_to_bytes(nm, rawv, lba_size)
    # total_writes (compat. resumen): usa escrituras del host en GB
    if result["total_host_writes"]:
        result["total_writes"] = result["total_host_writes"] / 1e9
    ...
    if attrs:
        ...
        if result["life_percent"] is None and result["health"] != "Desconocido":
    ...
    if nvme:
        ...
        # NVMe: 1 unidad de datos = 512000 bytes
        written = nvme.get("data_units_written", 0)
        read    = nvme.get("data_units_read", 0)
        result["total_writes"] = written * 512 * 1000 / 1e9  # GB approx
        if written:
            result["total_host_writes"] = written * 512000
        if read:
            result["total_host_reads"] = read * 512000
        ...
        if pct_used > 90:
            ...
            result["health"] = "Precaución"
        elif result["health"] == "Desconocido":
            result["health"] = "Bueno"
        ...
        for name, raw in nvme_attrs:
            result["attributes"].append({
                "id": "---", "name": name, "flag": "Bueno",
                "value": "-", "worst": "-", "thresh": "-", "raw": raw
            })

    return result

def _parse_lsblk_size(size_str):
    """Parsea tamaño de lsblk (ej: '500G', '1.8T', '512M') a GB"""
    if not size_str or size_str == "?":
        return None
    try:
        size_str = size_str.strip()
        multipliers = {"B": 1e-9, "K": 1e-6, "M": 1e-3, "G": 1.0, "T": 1000.0, "P": 1e6}
        suffix = size_str[-1].upper()
        if suffix in multipliers:
            return float(size_str[:-1]) * multipliers[suffix]
        ...
        def collect_children(node):
            ...
            for child in node.get("children", []):
                ...
                if ntype in ("part", "lvm", "crypt", "dm"):
                    ...
                    size   = child.get("size","?")
                    parsed_gb = _parse_lsblk_size(size)
                    part   = {
                        "name":    child.get("name",""),
                        "size":    size,
                        "fstype":  fstype,
...
def get_cpu_info():
    ...
    info = {
        "model": "Desconocido", "vendor": "", "family": "", "stepping": "",
    ...
    try:
        ...
        info["model"]       = d.get("model name", "Desconocido")
...
def get_gpu_info():
    ...
    result = []
    for g in gpus:
        name    = g.get("Device", g.get("SVendor","GPU Desconocida"))
        vendor  = g.get("Vendor", "")
        slot    = g.get("Slot",   "")
        gpu = {
            "name":    name,
...
def _detect_ram_channel(modules):
    if len(modules) >= 4: return "Cuádruple Canal"
    if len(modules) == 2:
        banks = set(m.get("Bank Locator","") for m in modules)
        return "Doble Canal" if len(banks) >= 2 else "Doble Canal (probable)"
    if len(modules) == 1: return "Canal Único"
    return "Desconocido"
...
class HealthBadge(QWidget):
    ...
    def __init__(self, status="Desconocido", percent=None, parent=None):
        super().__init__(parent)
        self.status  = status
...
class DiskButton(QPushButton):
    ...
    def __init__(self, disk, health="Desconocido", temp=None, parent=None):
        super().__init__(parent)
        self.disk   = disk
        self.health = health
        self.temp   = temp
...
class DiskInfoPanel(QWidget):
    ...
    def _build_ui(self):
        ...
        # ── Row 2: modelo / nombre del disco + botón copiar ──
        model_frame = QFrame()
        model_frame.setStyleSheet(
            "background-color: #161b22; border-bottom: 1px solid #30363d;"
        )
        model_l = QHBoxLayout(model_frame)
        model_l.setContentsMargins(16, 6, 16, 6)
        ...
        # Health badge
        self.health_badge = HealthBadge("Desconocido")
        ...
        left_fields = [
            ("Firmware",            "firmware"),
            ("Interfaz",            "interface"),
            ("Velocidad Interfaz",  "transfer_mode"),
            ("Versión ATA / SATA",  "standard"),
            ("Unidad",              "mountpoint"),
            ("Capacidad",           "capacity"),
        ...
        right_fields = [
            ("Lecturas Totales (Host)",   "total_host_reads"),
            ("Escrituras Totales (Host)", "total_host_writes"),
            ("Escrituras Totales (NAND)", "total_nand_writes"),
            ("Veces Encendido",           "power_on_count"),
        ...
        self.table.setHorizontalHeaderLabels([
            "ID", "Atributo", "Qué significa", "Estado", "Valor", "Peor", "Umbral", "Valor Raw"
        ])
    ...
    def load_disk(self, disk):
        ...
        self.lbl["transfer_mode"].setText(data.get("transfer_mode") or "--")
        self.lbl["standard"].setText(data.get("standard_str") or "--")
        self.lbl["capacity"].setText(data.get("capacity") or "--")
        self.lbl["features"].setText(data.get("features") or "--")
        ...
        self.lbl["total_host_reads"].setText(_fmt_bytes(data.get("total_host_reads")) or "--")
        self.lbl["total_host_writes"].setText(_fmt_bytes(data.get("total_host_writes")) or "--")
        self.lbl["total_nand_writes"].setText(_fmt_bytes(data.get("total_nand_writes")) or "--")

        # Table
        attrs = data["attributes"]
        self.table.setRowCount(len(attrs))
        for row, a in enumerate(attrs):
            # Resolver nombre en español + significado
            if a["id"] == "---":
                nombre_es, signif = SMART_NVME_ES.get(
                    a["name"], (a["name"], "Atributo del registro de salud NVMe."))
            else:
                try:
                    aid_int = int(a["id"])
                except (ValueError, TypeError):
                    aid_int = -1
                nombre_es, signif = SMART_ATTRS_ES.get(
                    aid_int,
                    (a["name"] or f"Atributo {a['id']}",
                     "Atributo específico del fabricante (sin descripción estándar)."))
            ...
            for col, (text, color) in enumerate(items):
                ...
                # Tooltip: nombre técnico original + significado completo
                if col == 1 and a["name"]:
                    item.setToolTip(f"{a['name']}\n\n{signif}")
                elif col == 2:
                    item.setToolTip(signif)
    ...
    def _save_disk_to_file(self):
        ...
        for idx, entry in enumerate(self._all_disks_data, 1):
            ...
            lines.append(f"  Lecturas (Host):     {_fmt_bytes(data.get('total_host_reads')) or '—'}")
            lines.append(f"  Escrituras (Host):   {_fmt_bytes(data.get('total_host_writes')) or '—'}")
            lines.append(f"  Escrituras (NAND):   {_fmt_bytes(data.get('total_nand_writes')) or '—'}")
    ...
    def _update_partition_view(self, partitions):
        ...
        if not partitions:
            no_lbl = QLabel("No se detectaron particiones")
...
class SystemInfoPanel(QWidget):
    ...
    def _populate(self, data):
        ...
        cpu_rows = [
            ...
            ("Microcódigo",     cpu["microcode"] or "—","#8b949e"),
        ...
        for gi, gpu in enumerate(data["gpus"]):
            ...
            gpu_rows = [
                ...
                ("Fabricante",    gpu["vendor"],               "#8b949e"),
                ("Slot PCI",      gpu["slot"],                 "#8b949e"),
                ("Controlador",   gpu["driver"] or "—",        "#58a6ff"),
                ("VRAM",          vram,                        "#3fb950"),
        ...
        mb_rows = [
            ...
            ("BIOS Fabricante", mb["bios_vendor"], "#8b949e"),
        ...
        ram_header_rows = [
            ...
            ("Ranuras usadas",   f"{ram['populated_slots']} / {ram['total_slots']}", "#58a6ff"),
        ...
        for i, mod in enumerate(ram["modules"]):
            ...
            mod_rows = [
                ...
                ("Número de Parte",   part,    "#8b949e"),
                ("Factor de Forma",   form,    "#8b949e"),
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("LinuxHWMonitor  v1.2 Rev 2")
    ...
    def _build_ui(self):
        ...
        self.status_msg.setStyleSheet("color: #ffffff; font-size: 13px; padding: 0 8px;")
        self.status.addWidget(self.status_msg, 1)

        # Reloj — permanente a la derecha
        self.status_time = QLabel("")
        self.status_time.setStyleSheet("color: #ffffff; padding: 0 8px; font-size: 13px;")
